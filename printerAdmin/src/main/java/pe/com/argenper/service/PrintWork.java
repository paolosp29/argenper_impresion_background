...
public class PrintWork extends JFrame {
    ...
    public PrintWork(String htmlDoc) {
        ...
        if (!htmlDoc.equalsIgnoreCase("impFisc")){	//si no es una impresion fiscal, entonces iniciar.
        	...
        	this.setConfDoc(new ConfigDocPrint()); //configuracion por defecto (por si el que usa la clase no se acuerda de pasarle la configuraciÃ³n de impresiÃ³n)
        }
    }
...
//        (new PrintWork()).setVisible(true);
	...
	private boolean iniciar(String htmlDoc)
	{
		try {
			...
			this.getError().setearErr("Error de iniciando trabajo de impresión. "+e.getMessage(), -100, "iniciar()", 0);
			return false;
		}
	}
